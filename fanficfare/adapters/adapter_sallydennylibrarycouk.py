...
class SallyDennyLibraryCoUkAdapter(BaseEfictionAdapter):

    @staticmethod
    def getSiteDomain():
        return 'www.sallydennylibrary.co.uk'
    ...
    def handleMetadataPair(self, key, value):
        if 'Pairing Type' in key:
            for val in re.split(r"\s*,\s*", value):
                self.story.addToList('schoolperiod', val)
        elif 'Pairing Type' in key:
            for val in re.split(r"\s*,\s*", value):
                self.story.addToList('schoolname', val)
        else:
            super(SallyDennyLibraryCoUkAdapter, self).handleMetadataPair(key, value)


def getClass():
    return SallyDennyLibraryCoUkAdapter
